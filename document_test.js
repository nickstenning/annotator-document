...
describe('document.document', function () {
...
describe('document.getDocumentMetadata', function () {
    var head;
    var tags;
    var metadata;
    ...
    beforeEach(function () {
        ...
        for (var name in FIXTURE_METADATA) {
            ...
            FIXTURE_METADATA[name].forEach(function (html) {
                head.insertAdjacentHTML('beforeend', html);
                tags[name].push(head.lastChild);
            });
        }
    ...
    it('does not have empty fields for nonexistent metadata', function () {
        tags.highwire.forEach(function (el) {
            head.removeChild(el);
        });
        tags.facebook.forEach(function (el) {
            head.removeChild(el);
        });
        metadata = doc.getDocumentMetadata();
    ...
    it('has a title derived from metadata, in preference order', function () {
        ...
        tags.eprints.forEach(function (el) {
            head.removeChild(el);
        });
        metadata = doc.getDocumentMetadata();
...
describe('document.absoluteUrl', function () {
